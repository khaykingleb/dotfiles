...
in
{
  ...
  # Resolve the configured 1Password references once during activation and cache
  # the resulting environment variables in the macOS login Keychain — the encrypted
  # credential store unlocked when the user logs in. Shells and MCP wrappers can
  # then read the cache without prompting 1Password whenever they start.
  # Reapplying the configuration refreshes the cached values.
  home.activation.refreshOnePasswordEnvironment = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    if [[ -v DRY_RUN ]]; then
      ...
      printf 'add-generic-password -U -s %s -a %s -w %s\n' \
        ${lib.escapeShellArg keychainEnvironment.serviceName} \
  ...
  programs.zsh.initContent = ''
    if onepassword_environment="$(/usr/bin/security find-generic-password -s ${lib.escapeShellArg keychainEnvironment.serviceName} -w 2>/dev/null)"; then
