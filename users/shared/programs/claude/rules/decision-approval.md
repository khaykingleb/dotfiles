...
- By default, do not execute terminal commands. Show the exact command in a shell code block, explain its purpose briefly, and let the user run it.
- Execute terminal commands only when the user explicitly asks the agent to run or execute them. Editing or implementation requests do not implicitly authorize terminal execution.
